...
__global__ void matrix_transpose_shared(int *input, int *output, int WIDTH) {
	...
	s_data[transposedIndex] = input[index];
	__syncthreads();
	
	output[index] = s_data[index];
	
	printf(
		"[cuda] threadIdx.x,y: (%d,%d), index: %d, input[index]_addr: %p, transposedIndex: %d, output[index]_addr: %p\n", 
		threadIdx.x, threadIdx.y, index, &input[index], index, &output[index]);
}
...
int main()
{
	...
	matrix_transpose_shared<<<Dg,Db, WIDTH*WIDTH>>> (dev_input, dev_output, WIDTH);
